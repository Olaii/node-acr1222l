...
var service = {
    ...
    readNDEF: function() {
      _log('readNDEF called');  
      return new Promise(function(resolve, reject) {
         if(cardPresent) {
             readNDEF().then(resolve, function(err) {
              _log('Card removed during read. Please present card to the reader');
    ...
    sendBuffer: function(buffer, addr) {
        _log('sendBuffer called with Addr: ' + addr + ' and buffer: ' + buffer);
        return new Promise(function (resolve, reject) {
            if(cardPresent){
                sendBuffer(buffer, addr).then(resolve, reject);
            } else {
                writeRequested = true;
                writeBuffer = buffer;
                writeAddr = addr;
                writeCallbackObj = {resolve: resolve, reject: reject};
            }
        });
    },
...
function sendBuffer(buffer, addr) {
    return new Promise(function(resolve, reject) {
        _connect(util.CONN_MODE(service.reader), util.CARD_PROTOCOL)
            .then(function() {
                _log('SendBufffer sending to card');
                const WRITE = new Buffer([0xFF, 0xD6, 0x00, addr, buffer.length, ...buffer]);
                service.reader.transmit(WRITE, 40, connectionProtocol, function(err, data) {
                   if (err) {
                       _log('Write failed with error msg:' + err);
                       reject(err);
                   } else {
                       if(data[0] == 0x90) {
                           _log('Write successfull')
                           resolve(data.slice(0, -2));  // remove 90 00 from UUID as it's part of a success frame
                       } else if (data[0] == 0xfe){
                           _log('Write failed with error code 0xfe');
                           reject(new Error('Write failed! Card locked!'));
                       } else {
                           _log('Write failed with error code: ' + data);
                           reject(new Error('Write failed! Error code: ' + data));
                       }
                   }
                });
            });
...
function getNDEFData(data) {
    return new Promise(function(resolve, reject) {
        if (data.indexOf(0xd1) == -1) {
            ...
            var end_terminator = 0xfe;
            if (data.indexOf(0xfe) == -1) {
                end_terminator = 0x00;
            }
            var ndef_data = data.slice(data.indexOf(0xd1), data.indexOf(end_terminator)).toJSON();
...
function handleCardPresentMessage() {
    ...
    if(writeRequested) {
        sendBuffer(writeBuffer, writeAddr).then(function(response) {
            writeRequested = false;
            if (writeCallbackObj) writeCallbackObj.resolve(response);
            writeCallbackObj = null;
            writeBuffer = null;
            writeAddr = null;
        }, function (err) {
            writeRequested = false;
            ...
            writeBuffer = null;
            writeAddr = null;
            _log('Send Buffer has failed.', err)
        });
    }
